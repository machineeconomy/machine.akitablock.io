<template>
  <div class="machine">
    ...
    <div v-if="connected" class="info">
      <div :class="status">
        <span :type="getStatusColor(status)">{{status}}</span>
      </div>
    </div>
    <div v-else class="info not_connected">
      <span :type="getStatusColor('not_connected')">not connected</span>
      <p>
        connecting...
...
export default {
  ...
  methods: {
    getImgUrl() {
      ...
      //return images("./" + this.name + ".png"); <-- use this for dynamic img
